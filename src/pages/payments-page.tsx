...
export function PaymentsPage() {
  ...
  const installments = useAsyncData(fetchOpenInstallments, [], { cacheKey: 'payments:open-installments', staleTime: 60 * 1000, gcTime: 10 * 60 * 1000 })
  const recentPayments = useAsyncData(listRecentPayments, [], { cacheKey: 'payments:recent', staleTime: 60 * 1000, gcTime: 10 * 60 * 1000 })
  const settingsLoader = useCallback(() => getActiveLoanSettings(profile?.id), [profile?.id])
  const settings = useAsyncData(settingsLoader, null, { cacheKey: profile?.id ? `loan-settings:${profile.id}` : undefined, staleTime: 5 * 60 * 1000, gcTime: 20 * 60 * 1000 })
