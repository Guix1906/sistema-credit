...
begin
  ...
  values (
    v_owner_id,
    v_client_id,
    nullif(p_loan->>'route_id', '')::uuid,
    nullif(p_loan->>'collector_id', '')::uuid,
    (p_loan->>'principal_amount')::numeric,
    (p_loan->>'interest_amount')::numeric,
    (p_loan->>'total_amount')::numeric,
    0,
    (p_loan->>'total_amount')::numeric,
    (p_loan->>'issued_at')::date,
    (p_loan->>'first_due_date')::date,
    (p_loan->>'final_due_date')::date,
    p_loan->>'payment_frequency',
    (p_loan->>'term_days')::integer,
  ...
  if p_cashbox_id is not null then
    ...
    set current_balance = current_balance - (p_loan->>'principal_amount')::numeric
    where id = p_cashbox_id;
  end if;

  insert into public.audit_logs (owner_id, actor_id, table_name, record_id, action, new_data)
  values (v_owner_id, v_owner_id, 'loans', v_loan_id, 'insert', jsonb_build_object('client_id', v_client_id, 'total_amount', p_loan->>'total_amount'));
...
begin
  ...
  insert into public.payments (
    owner_id, client_id, loan_id, installment_id, cashbox_id, amount,
    late_fee_amount, paid_at, payment_method, notes
  )
  values (
    v_owner_id, v_loan.client_id, v_loan.id, v_installment.id, p_cashbox_id,
    p_amount_paid, greatest(p_late_fee_amount, 0), p_payment_date, p_payment_method, p_notes
  )
  ...
  update public.loans
  set paid_amount = v_loan_paid,
      remaining_amount = v_loan_remaining,
      status = case when v_loan_remaining = 0 then 'paid' else status end
  where id = v_loan.id;
  ...
  if p_cashbox_id is not null then
    ...
    values (v_owner_id, p_cashbox_id, v_payment_id, 'inflow', p_amount_paid, 'Recebimento de parcela ' || v_installment.installment_number);
  ...
  insert into public.audit_logs (owner_id, actor_id, table_name, record_id, action, new_data)
  values (v_owner_id, v_owner_id, 'payments', v_payment_id, 'insert', jsonb_build_object('installment_id', v_installment.id, 'amount', p_amount_paid));
...
grant execute on function public.register_installment_payment(uuid, numeric, timestamptz, text, uuid, text, numeric) to authenticated;
